...
# 4. CSS PREMIUM E MARCA D'ÁGUA S.A.I.D.
st.markdown("""
    <style>
...
if st.session_state.page == "home":
    ...
    if pergunta:
        with st.spinner("S.A.I.D. consultando base de dados..."):
            try:
                ...
                st.error("O consultor S.A.I.D. está processando um alto volume de dados. Tente em instantes.")
    ...
    if st.button("CONTRATAR LIMPEZA"):
        if lote_servico: st.success(f"Limpeza para o Lote {lote_servico} agendada para {data_servico.strftime('%d/%m/%Y')}!")
    st.markdown("---")
    ...
    if st.button("SOLICITAR ORÇAMENTO"):
        if lote_servico: st.info(f"Orçamento enviado para o Lote {lote_servico} para {data_servico.strftime('%d/%m/%Y')}.")
    ...
    with st.form("form_cad"):
        ...
        rg = c1.text_input("RG"); nasc = c2.date_input("DATA NASCIMENTO")
        wa = c1.text_input("WHATSAPP"); mail = c2.text_input("E-MAIL")
        renda = st.number_input("RENDA", min_value=0.0)
        aceite = st.checkbox("CONCORDO LGPD")
    ...
    st.date_input("DATA DA VISITA")
    if st.button("CONFIRMAR AGENDAMENTO"): st.success("Visita agendada!")
